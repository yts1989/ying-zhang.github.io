...
Google的Borg系统是一个群集管理器。它在多个万台机器规模的集群上运行着来自几千个不同的应用的几十万个作业。

Borg通过准入控制、高效的任务装箱、超售、共享机器和进程级别的性能隔离达到了高利用率。它为高可用应用提供的运行时特性包括减少故障恢复时间和减少关联故障概率的调度策略。Borg提供了声明式的作业描述语言、名字服务集成、实时作业监控和分析及模拟系统行为的工具，这些简化了用户的使用。
